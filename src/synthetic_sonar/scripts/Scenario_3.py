...
class TabbedApp:
    def __init__(self, root,sonar_subscriber,robot_pose_subscriber):
        ...
        self.cmd_vel_publisher = rospy.Publisher('pioneer/cmd_vel', Twist, queue_size=10)
        ...
        self.root = root
        self.root.title("EKF with Forward Model")
        ...
        self.notebook.add(self.tab5, text="Tab 5 - Error")
        ...
        scenario_3 = Button(self.tab1, text="Scenario 3", command=self.scenario_3)
        scenario_3.grid(row=1, column=3, pady=5)
    ...
    def create_plots(self,tab1_img=np.random.rand(10, 10),tab2_img=np.random.rand(10, 10),tab4_img=np.random.rand(10, 10),tab5_img=np.random.rand(10, 10)):
        ...
        self.ax1.text(self.x_robot_chart, self.y_robot_chart, label_text, color='green', verticalalignment='bottom', horizontalalignment='left')
        ...
        #self.plot_image(self.tab5, self.figure5, self.ax5, tab5_img, 'coolwarm', "Covariance",vmin=int(np.min(tab5_img)),vmax=0)
        #canvas5 = FigureCanvasTkAgg(self.figure5, master=self.tab5)
        #canvas5.draw()
        #canvas5.get_tk_widget().grid(row=0, column=0, pady=5)
    ...
    def next_step(self,data=None):
        ...
        f_score_ekf=self.calculate_f1_score(ekf_map.img,gt_map.img_small,0.5,0.5)
        f_score_inv=self.calculate_f1_score(inv_map.img,gt_map.img_small,0.5,0.5)
    ...
    def sonar_update(self):
        ...
        for topic in self.sonar_subscriber.sonar_topics:
            ...
            I_all_set = set()
            J_all_set.clear()#here unique cells (iy,ix) will be saved
            I_all_set.clear()
            print(topic)
            ...
            for index, (ix,iy) in enumerate(I_all):
                d_idx=d-index*(d/len(I_all))
                ekf_map.p_false_alarm[iy,ix]=self.prob_fa.get(round(d_idx,2), 0.005)
                ekf_map.p_true_detection[iy,ix]=self.p_true_detection_gaussian.get(i,0.7)
                part_1=ekf_map.state_to_prob(ekf_map.mu[iy,ix])*(1-ekf_map.p_true_detection[iy,ix])
                part_2=(1-ekf_map.state_to_prob(ekf_map.mu[iy,ix]))*(1-ekf_map.p_false_alarm[iy,ix])
                ekf_map.free_perceived_prob[iy,ix]=part_1+part_2 #p_miss
    ...
    def scenario_3(self,distance=4):
...
if __name__ == '__main__':
    try:
        ...
        inv_map = OccupancyGridMap(name='INV_map',resolution=0.2, width=8, height=8, center_x=0, center_y=0, use_subscriber=False,use_sub_topic=None)
        ekf_map = OccupancyGridMap(name='EKF_map',resolution=0.2, width=8, height=8, center_x=0, center_y=0, use_subscriber=False,use_sub_topic=None,ekf=True)
